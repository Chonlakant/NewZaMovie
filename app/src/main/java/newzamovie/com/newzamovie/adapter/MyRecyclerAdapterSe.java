...
public class MyRecyclerAdapterSe extends RecyclerView.Adapter<MyRecyclerAdapterSe.ContactViewHolder> {

    private ArrayList<Se> contactList = new ArrayList<>();
    Context context;
    ...
    public MyRecyclerAdapterSe(Context context, ArrayList<Se> contactList) {
    ...
    public void onBindViewHolder(ContactViewHolder contactViewHolder, int i) {
       // listMain ci = contactList.get(i);
        Se item =  contactList.get(i);

        contactViewHolder.movie_name.setText(item.getSeries().get(i).getTitle());

        Picasso.with(context)
                .load(item.getSeries().get(i).getUrlcover())
                .fit().centerCrop()
